...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    final_acked = INIT_SEQNO + content_len
    ...
    while win_left_edge < INIT_SEQNO + content_len:
        ...
        if r:
            ...
            if(ack_msg.ack > last_acked):
                ...
                win_right_edge = min(win_left_edge+win_size, final_acked)
                #transmit_entire_window_from(win_left_edge)
                if(final_acked == last_acked):
                    break

        else:
            print("TIMEOUT: Attempting to retransmit packet " + str(win_left_edge))
            transmit_one()
